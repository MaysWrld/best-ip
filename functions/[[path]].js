...
// 增强的 isIPv6 检查，确保准确识别所有 IPv6 格式
const isIPv6 = (ip) => {
    const trimmedIp = ip.trim();
    // 1. 必须包含冒号且不包含点号 (排除 IPv4)
    if (trimmedIp.includes(':') && !trimmedIp.includes('.')) {
        // 2. 使用正则表达式检查是否符合基本的 IPv6 字符集和结构 (支持 :: 缩写)
        // 这个正则表达式可以匹配大多数合法的 IPv6 地址，包括缩写形式。
        return /^(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$|^::(?:[0-9a-fA-F]{1,4}:){0,6}[0-9a-fA-F]{1,4}$|^[0-9a-fA-F]{1,4}::(?:[0-9a-fA-F]{1,4}:){0,5}[0-9a-fA-F]{1,4}$|^[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){1}::(?:[0-9a-fA-F]{1,4}:){0,4}[0-9a-fA-F]{1,4}$|^[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){2}::(?:[0-9a-fA-F]{1,4}:){0,3}[0-9a-fA-F]{1,4}$|^[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){3}::(?:[0-9a-fA-F]{1,4}:){0,2}[0-9a-fA-F]{1,4}$|^[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){4}::(?:[0-9a-fA-F]{1,4}:){0,1}[0-9a-fA-F]{1,4}$|^[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){5}::[0-9a-fA-F]{1,4}$|^[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){6}::$/.test(trimmedIp);
    }
    return false;
};
